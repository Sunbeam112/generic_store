...
import ua.sunbeam.genericstore.api.controller.InventoryController;
...
public class OrderItemsService {
    ...
    public OrderItemsService(OrderService orderService, ProductService productService,
                             OrderItemsRepository orderItemsRepository, InventoryController inventoryController) {
    ...
    public boolean addItemsToOrder(@Valid @Min.List({@Min(value = 1), @Min(value = 1)}) List<ProductToOrderBody> products, Long orderId, BindingResult result) throws IllegalArgumentException {
        ...
        for (ProductToOrderBody product : products) {
            if (product.getProductID() > 0 && product.getQuantity() > 0) {
                ...
                boolean isInStock = inventoryController.getProductQuantity(product.getProductID()) > product.getQuantity();
